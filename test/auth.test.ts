...
test("Helix permissions separate reads, triggering, management, and history deletion", async () => {
  ...
  await request(steering).post("/api/steering/decisions").send({}).expect(403);
  await request(steering).post("/runs").send({ title: "No broad trigger" }).expect(403);

  const receiver = appFor(principal(["helix.steering.receive"]));
  await request(receiver).post("/api/steering/decisions").send({}).expect(400);
  await request(receiver).post("/api/steering/actions").send({}).expect(403);
  await request(receiver).post("/runs").send({ title: "No broad trigger" }).expect(403);
