...
- Helix owns the non-secret Steering URL shown under **Connections**. A saved value overrides `ACME_STEERING_URL`; clearing the override returns to startup configuration. Tokens remain server-side and may be sent only to trusted origins.
