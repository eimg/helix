...
| Acme Intel | `~/Desktop/acme/acme-intel` | Optional think-lab; may study allowlisted Helix run/PR-review facts without mutating Helix. |
...
- Helix owns the non-secret Steering URL shown under **Connections**. A saved value overrides `ACME_STEERING_URL`; clearing the override returns to startup configuration. `helix init` seeds a commented local-suite default in `.helix/.env.example`; Connections may one-click-save that same URL. Tokens remain server-side and may be sent only to trusted origins.
